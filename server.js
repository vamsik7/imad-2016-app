...
function loadCatArticles (articleData) {
...
app.get('/get-categories', function (req, res) {
   // make a select request
   // return a response with the results
   pool.query('SELECT * FROM categories ORDER BY name DESC', function (err, result) {
      if (err) {
          res.status(500).send(err.toString());
      } else {
          res.send(JSON.stringify(result.rows));
...
app.get('/categories/:catName', function (req, res) {
  // SELECT * FROM article WHERE title = '\'; DELETE WHERE a = \'asdf'
  pool.query("SELECT * FROM articles WHERE category = $1", [req.params.catName], function (err, result) {
    if (err) {
        res.status(500).send(err.toString());
    } else {
        if (result.rows.length === 0) {
            res.status(404).send('Category is empty or not found');
        } else {
            var articleData = result.rows[0];
            res.send(loadCatArticles(articleData));
        }
    }
  });
});
